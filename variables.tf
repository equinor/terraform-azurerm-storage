...
variable "advanced_threat_protection_enabled" {
  description = "Should Defender for Storage (classic) advanced threat protection be enabled for this Storage account? After February 5, 2025, you can no longer enable Defender for Storage (classic) in most scenarios. The only exception is for subscriptions that already have Defender for Storage (classic) enabled."
  type        = bool
  default     = false
  nullable    = false
}
